...
public class Y3SeasonBuilder
    ...
    private Long toFirst(Long season, Date openDate) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(openDate);
        if (cal.get(6) == 1) {
            String s = cal.get(1) + "001";
            season = Long.valueOf(Long.parseLong(s));
        }
        return season;
    }
    ...
    public List<LotterySaleTime> create(Lottery lottery, List<LotterySaleRule> rules, List<LotteryCloseRule> closes, Date begin, int days) {
        /**jd-gui
         ...
         season = toFirst(season, openDate);
         }
