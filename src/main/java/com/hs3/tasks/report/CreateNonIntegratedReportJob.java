...
public class CreateNonIntegratedReportJob implements Job {
    ...
    public void execute(JobExecutionContext arg0) throws JobExecutionException {
        try {
            logger.info("生成代理报表任务启动！！！！");
            this.cpsReportService.addAgentsReportWhenNotExists(DateUtils.format(DateUtils.addDay(new Date(), -1), "yyyy-MM-dd"));
        } catch (Exception e) {
            logger.error("生成代理报表任务异常！" + e.getMessage(), e);
        } finally {
            logger.info("生成代理报表任务结束！！！！");
        }
    }
