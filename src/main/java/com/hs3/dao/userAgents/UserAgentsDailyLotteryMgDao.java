...
@Repository("userAgentsDailyLotteryMgDao")
public class UserAgentsDailyLotteryMgDao extends BaseDao<UserAgentsDailyLotteryMg> {


    public void save(UserAgentsDailyLotteryMg m) {
        this.dbSession.update("INSERT INTO t_user_agents_daily_lottery_mg(reportDate,account,parentAccount,lotteryId,lotteryName,distributionAmount,actualSalesVolume,actualActiveNumber,programId,programName,salesVolume,proportion,activeNumber,cycle,status,createTime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                new Object[]{m.getReportDate(), m.getAccount(), m.getParentAccount(), m.getLotteryId(), m.getLotteryName(), m.getDistributionAmount(), m.getActualSalesVolume(), m.getActualActiveNumber(), m.getProgramId(), m.getProgramName(), m.getSalesVolume(), m.getProportion(), m.getActiveNumber(), m.getCycle(), m.getStatus(), new Date()});
    }

    public int update(UserAgentsDailyLotteryMg m) {
        return this.dbSession.update("UPDATE t_user_agents_daily_lottery_mg SET status=?,modifyTime=?,operator=?,remarks=? WHERE id=?",
    ...
    public List<UserAgentsDailyLotteryMg> userAgentsDailyLotteryMgList(String account, String parentAccount, String programName, String begin, String end, Date startTime, Date endTime, Integer status, Page p) {
        ...
        String sql = "SELECT ";
        sql = sql + " * ";
        sql = sql + " from t_user_agents_daily_lottery_mg ";
        sql = sql + " where 1 = 1 ";
        ...
        return this.dbSession.list(sql, arg, UserAgentsDailyLotteryMg.class, p);
    }
