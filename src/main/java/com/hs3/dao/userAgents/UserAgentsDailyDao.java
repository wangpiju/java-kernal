...
@Repository("userAgentsDailyDao")
public class UserAgentsDailyDao extends BaseDao<UserAgentsDaily> {


    public void save(UserAgentsDaily m) {
        this.dbSession.update("INSERT INTO t_user_agents_daily(programName,salesVolume,proportion,activeNumber,cycle,status,hint) VALUES(?,?,?,?,?,?,?)",
                new Object[]{m.getProgramName(), m.getSalesVolume(), m.getProportion(), m.getActiveNumber(), m.getCycle(), m.getStatus(), m.getHint()});
    }


    public int update(UserAgentsDaily m) {
        return this.dbSession.update("UPDATE t_user_agents_daily SET salesVolume=?,proportion=?,activeNumber=?,cycle=?,status=?,hint=? WHERE id=?",
    ...
    public List<UserAgentsDaily> userAgentsDailyList(String programName, Integer status, Integer sorting, Page p) {
        ...
        String sql = "SELECT ";
        sql = sql + " * ";
        sql = sql + " from t_user_agents_daily ";
        sql = sql + " where 1 = 1 ";
        ...
        return this.dbSession.list(sql, arg, UserAgentsDaily.class, p);
    }
