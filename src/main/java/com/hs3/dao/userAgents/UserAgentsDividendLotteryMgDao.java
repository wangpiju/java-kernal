...
@Repository("userAgentsDividendLotteryMgDao")
public class UserAgentsDividendLotteryMgDao extends BaseDao<UserAgentsDividendLotteryMg> {

    public void save(UserAgentsDividendLotteryMg m) {

        this.dbSession.update("INSERT INTO t_user_agents_dividend_lottery_mg(reportDate,account,parentAccount,distributionAmount,actualNegativeProfit,actualActiveNumber,programId,programName,negativeProfit,proportion,activeNumber,cycle,status,createTime,lotteryId,lotteryName) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                new Object[]{m.getReportDate(), m.getAccount(), m.getParentAccount(), m.getDistributionAmount(), m.getActualNegativeProfit(), m.getActualActiveNumber(), m.getProgramId(), m.getProgramName(), m.getNegativeProfit(), m.getProportion(), m.getActiveNumber(), m.getCycle(), m.getStatus(), new Date(), m.getLotteryId(), m.getLotteryName()});

    }

    public int update(UserAgentsDividendLotteryMg m) {
        return this.dbSession.update("UPDATE t_user_agents_dividend_lottery_mg SET status=?,modifyTime=?,operator=?,remarks=? WHERE id=?",
    ...
    public List<UserAgentsDividendLotteryMg> userAgentsDividendLotteryMgList(String account, String parentAccount, String programName, String begin, String end, Date startTime, Date endTime, Integer status, Page p) {
        ...
        String sql = "SELECT ";
        sql = sql + " * ";
        sql = sql + " from t_user_agents_dividend_lottery_mg ";
        sql = sql + " where 1 = 1 ";
        ...
        return this.dbSession.list(sql, arg, UserAgentsDividendLotteryMg.class, p);
    }
