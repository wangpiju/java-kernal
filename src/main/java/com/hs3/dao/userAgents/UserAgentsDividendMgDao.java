...
@Repository("userAgentsDividendMgDao")
public class UserAgentsDividendMgDao extends BaseDao<UserAgentsDividendMg> {

    public void save(UserAgentsDividendMg m) {
        this.dbSession.update("INSERT INTO t_user_agents_dividend_mg(reportDate,account,parentAccount,distributionAmount,actualNegativeProfit,actualActiveNumber,programId,programName,negativeProfit,proportion,activeNumber,cycle,status,createTime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                new Object[]{m.getReportDate(), m.getAccount(), m.getParentAccount(), m.getDistributionAmount(), m.getActualNegativeProfit(), m.getActualActiveNumber(), m.getProgramId(), m.getProgramName(), m.getNegativeProfit(), m.getProportion(), m.getActiveNumber(), m.getCycle(), m.getStatus(), new Date()});
    }

    public int update(UserAgentsDividendMg m) {
        return this.dbSession.update("UPDATE t_user_agents_dividend_mg SET status=?,modifyTime=?,operator=?,remarks=? WHERE id=?",
    ...
    public List<UserAgentsDividendMg> userAgentsDividendMgList(String account, String parentAccount, String programName, String begin, String end, Date startTime, Date endTime, Integer status, Page p) {
        ...
        String sql = "SELECT ";
        sql = sql + " * ";
        sql = sql + " from t_user_agents_dividend_mg ";
        sql = sql + " where 1 = 1 ";
        ...
        return this.dbSession.list(sql, arg, UserAgentsDividendMg.class, p);
    }
